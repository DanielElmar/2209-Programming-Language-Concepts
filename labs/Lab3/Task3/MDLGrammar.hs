...
happyExpList = Happy_Data_Array.listArray (0,42) ([39968,32768,2048,39,1024,12,384,256,33792,19,768,0,0,0,0,0,0,0,4996,8192,57602,4,128,0,9216,19984,0,8,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseMDL","Exp","Direction","forward","digit","int","l","r","rotate","obstacle","if","then","else","'('","')'","';'","%eof"]
...
action_4 (8) = happyShift action_14
...
happyReduction_2 (HappyTerminal (TokenInt p happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Forward happy_var_2
	)
...
happyReduction_3 (HappyTerminal (TokenDigit p happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Forward happy_var_2
	)
...
happyReduction_5 (HappyTerminal (TokenDigit p happy_var_2))
	_
	 =  HappyAbsSyn4
		 (Obstacle happy_var_2
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenForward p -> cont 6;
	TokenDigit p happy_dollar_dollar -> cont 7;
	TokenInt p happy_dollar_dollar -> cont 8;
	TokenLeft p -> cont 9;
	TokenRight p -> cont 10;
	TokenRotate p -> cont 11;
	TokenObstacle p -> cont 12;
	TokenIf p -> cont 13;
	TokenThen p -> cont 14;
	TokenElse p -> cont 15;
	TokenLParen p -> cont 16;
	TokenRParen  p -> cont 17;
	TokenSeq p -> cont 18;
	_ -> happyError' ((tk:tks), [])
	}
